    ...
    context "on #{os}" do
      ...
      describe 'Change Defaults' do
        ...
        context 'enable_nagios' do
          ...
          it do
            expect(exported_resources).to contain_nagios_service(
              'routing.example.com_BGP_NEIGHBOUR_192.0.2.2'
            ).with(
          ...
          it do
            expect(exported_resources).to contain_nagios_service(
              'routing.example.com_BGP_NEIGHBOUR_192.0.2.3'
            ).with(
              host_name: 'routing.example.com',
          ...
          it do
            expect(exported_resources).to contain_nagios_service(
              'routing.example.com_BGP_NEIGHBOUR_2001:DB8::2'
            ).with(
              host_name: 'routing.example.com',
        ...
        context 'enable_nagios enable_advertisements false' do
          ...
          it { is_expected.to compile }
          it do
            expect(exported_resources).not_to contain_nagios_service(
              'routing.example.com_BGP_NEIGHBOUR_192.0.2.2'
            ).with(
              host_name: 'routing.example.com',
              service_description: 'BGP_NEIGHBOUR_192.0.2.2',
